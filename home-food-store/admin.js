(() => {
  'use strict';
  const CFG = window.FOOD_STORE_CONFIG || { apiBase:'/api', storagePrefix:'home_food_store_v1' };
  const API = CFG.apiBase || '/api';
  const tokenKey = `${CFG.storagePrefix || 'home_food_store_v1'}_admin_token`;
  ...
  async function downloadBackup(){try{const data=await api('/backup');const blob=new Blob([JSON.stringify(data,null,2)],{type:'application/json'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`food-store-backup-${new Date().toISOString().slice(0,10)}.json`;a.click();URL.revokeObjectURL(a.href);toast('تم تجهيز النسخة الاحتياطية');}catch(err){toast(err.message,'error');}}
  ...
  function bind(){
    ...
    $('#productImageFile').onchange=()=>uploadImage($('#productImageFile'),$('#productImageUrl'),$('#productForm').mainImagePublicId,'food-store/products');
    $('#categoryImageFile').onchange=()=>uploadImage($('#categoryImageFile'),$('#categoryImageUrl'),null,'food-store/categories');
    $('#galleryImageFile').onchange=()=>uploadImage($('#galleryImageFile'),$('#galleryImageUrl'),$('#galleryForm').cloudinaryPublicId,'food-store/gallery');
  }
